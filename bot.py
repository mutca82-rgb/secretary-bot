...
# ─── Обновление профиля пользователя через AI ────────────────────────────────
...
# ─── Построение контекста для DeepSeek ────────────────────────────────────────
...
# ─── Парсинг события из текста через AI ─────────────────────────────────────
...
async def cmd_event(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if len(parts) < 2:
        await update.message.reply_text("❌ Минимум: название и дата.\n`/event Встреча; 2025-06-01 14:00`", parse_mode="Markdown")
        return
    title = parts[0]
...
# ─── Общая логика обработки текста ⚡ БЕЗ parse_mode для AI-ответов ─────────
...
async def process_message(user_id: int, text: str, update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ⚠️ Отправляем БЕЗ parse_mode, чтобы избежать ошибок Markdown
...
# ─── Обработчик голосовых (заглушка) ────────────────────────────────────────

async def handle_voice(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text("🎤 Голосовые пока не поддерживаются с DeepSeek. Напиши текстом.")
